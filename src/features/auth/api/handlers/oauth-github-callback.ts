...
import { User, OAuthState, UsersData, UserIdentity } from '@/features/auth/types';

export async function handleGitHubOAuthCallback(request: NextRequest) {
  try {
    ...
    const oauthState = await authPersistence.validateOAuthState(stateToken, 'github');
    if (!oauthState) {
    ...
    const existingOAuthIdentity = Object.values(usersData.identities).find(
      (ident): ident is UserIdentity =>
        ident.provider === 'github' && ident.providerId === providerId
    );
    ...
    if (existingOAuthIdentity) {
      // User already has GitHub identity - just log them in
      ...
      if (existingEmailIdentity) {
        // User exists with email - link GitHub identity to existing user
        ...
        // Create new GitHub identity linked to existing user
        ...
        // New user - create user and GitHub identity
        const now = new Date().toISOString();
