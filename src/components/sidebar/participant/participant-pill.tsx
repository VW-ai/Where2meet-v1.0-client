...
export function ParticipantPill({ participant, onClick }: ParticipantPillProps) {
  ...
  return (
    <div className="relative w-full py-2 pl-6 pr-1 group">
      <div
        className="relative z-10 flex items-center cursor-pointer transition-transform hover:-translate-y-0.5"
        onClick={onClick}
      >
        {/* --- THE TAIL --- 
        ...
        */}
        <div className="relative flex-1 flex items-center justify-between bg-white border-2 border-border rounded-2xl px-4 py-2.5 shadow-sm hover:shadow-md transition-all z-10">
          {/* Left side - Info */}
          <div className="flex flex-col gap-0.5 min-w-0 flex-1 mr-4">
            <div className="flex items-center gap-2">
              ...
              {participant.fuzzyLocation && (
                <span
                  className="inline-flex items-center justify-center bg-muted/50 rounded-full p-0.5"
